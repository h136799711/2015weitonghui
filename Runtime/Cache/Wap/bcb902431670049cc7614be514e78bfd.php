...
<link type="text/css" rel="stylesheet" href="/Public/Wap/css/store/css/touch_index_1.css">
</head>
<body>
<div id="top"></div>
...
<?php if(is_array($cats)): $i = 0; $__LIST__ = $cats;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i; if ($i > 6) {$k = $i % 6 + 1;} else {$k = $i;} ?>
	<div class="m-floor">
	<a class="tit bgf<?php echo ($k); ?>" href="<?php echo U('Store/products',array('token' => $_GET['token'], 'catid' => $hostlist['id'], 'wecha_id' => $wecha_id));?>">
	<i></i><?php echo ($hostlist["name"]); ?></a>
