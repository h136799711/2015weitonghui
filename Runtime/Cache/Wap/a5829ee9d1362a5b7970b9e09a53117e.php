...
<link href="/Public/Wap/css/guajiang/css/activity-style.css" rel="stylesheet" type="text/css">
</head>
...
<?php if($Dazpan['end'] == 1): ?><div class="activity-lottery-end" >
    <div  class="main" >
    <div class="banner"><img src="<?php if(!$Dazpan['endpicurl']){ ?>/Public/Wap/css/guajiang/images/activity-lottery-end2.jpg<?php }else{ echo ($Dazpan["endpicurl"]); } ?>" /></div>
...
<?php else: ?>
    ...
    <div id="outercont"  >
        <div id="outer-cont">
            <div id="outer"><img src="/Public/Wap/css/guajiang/images/activity-lottery-5.png"></div>
        </div>
        <div id="inner-cont">
            <div id="inner" <?php if($wecha_id == ''): ?>onclick="alert('您不能参加此活动');"<?php else: endif; ?>><img src="/Public/Wap/css/guajiang/images/activity-lottery-2.png"></div>
        </div>
    </div>
    <?php  } endif; ?>
...
<div class="content">
    ...
    <div class="box">
        ...
        <div class="Detail">   
            ...
            <p>兑奖<?php echo ($Dazpan["renamesn"]); ?>：<span class="red" id="sncode" ><?php echo ($Dazpan["sn"]); ?></span>请您牢记及尽快与我们联系</p>
            <p class="red" id="red"><?php echo ($Dazpan["sttxt"]); ?></p>
            <!-- <?php if($record["phone"] == '' and $record["sendstatus"] == 0): ?>-->
           <!--  <p style="display:none;">
            <input name=""  class="px" id="tel" value="" type="text" placeholder="请输入您的<?php echo ($Dazpan["renametel"]); ?>">
            <input name=""  class="px" id="wechaname" value="" style="display:none;" type="text" placeholder="请输入您的微信号">
...
<div class="boxcontent boxyellow">
    ...
    <div class="title-green"><span>奖项设置：</span></div>
		 ...
		 <p><?php if($Dazpan["canrqnums"] != 0): ?>每人最多抽奖次数:<?php echo ($Dazpan["canrqnums"]); endif; if($Dazpan["daynums"] != 0): ?>每天只能抽<?php echo ($Dazpan["daynums"]); ?>次<?php endif; ?> <?php if($Dazpan["usenums"] > 0): ?>今天已抽取 <span class="red" id="usenums"><?php echo ($Dazpan["usenums"]); ?></span> 次<?php endif; ?></p>
...
<?php if($wecha_id != ''): ?>$("#inner").click(function() {
        ...
        if (running) return;
        if (count >= <?php echo ($Dazpan["canrqnums"]); ?> && <?php echo ($Dazpan["canrqnums"]); ?> >0) {
       ...
       $.ajax({
         ...
         url     : "<?php echo U('Wap/Lottery/getajax',array('id'=>$Dazpan['lid'],'rid'=>$Dazpan['rid'],'token'=>$Dazpan['token'],'oneid'=>$Dazpan['wecha_id']));?>",
         dataType: "json",
...
function() {
    ...
    $.post("<?php echo U('Wap/Lottery/add');?>", submitData,
    function(data) {
...
function () {
	...
	$.post("<?php echo U('Wap/Lottery/exchange');?>", submitData,
	function (data) {
		if (data.success == true) {
			alert(data.msg);
...
window.shareData = {  
            ...
            "sendFriendLink": "<?php echo ($cfg_siteUrl); echo U('Lottery/index',array('token'=>$token,'id'=>$Dazpan['id'],'type'=>1));?>",
