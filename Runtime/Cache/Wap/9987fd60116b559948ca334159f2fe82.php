...
<script src="/Public/Wap/css/store/js/jquery-1.9.1.min.js" type="text/javascript"></script>
<script src="/Public/Wap/css/store/js/jquery.lazyload.js" type="text/javascript"></script>
<script src="/Public/Wap/css/store/js/notification.js" type="text/javascript"></script>
<script src="/Public/Wap/css/store/js/swiper.min.js" type="text/javascript"></script>
<script src="/Public/Wap/css/store/js/main.js" type="text/javascript"></script>
<link type="text/css" rel="stylesheet" href="/Public/Wap/css/store/css/style_touch.css">
</head>
<script>
...
<form method="post" action="<?php echo U('Store/ordersave',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id']));?>" id="FromID">
            ...
            <li class="addr-info">
                <ul class="addr-addnew-form addr-edit-form" id="addr-edit-form" style="display: none1;">
                    <li>
                        ...
                        <label>详细地址：</label><span><textarea name="address" id="address" rows="5" placeholder="输入您的收货地址" style="width:100%"><?php echo ($fans["address"]); ?></textarea></span>
                    </li>
                </ul>
                <label>联系方式保存到用户库</label>
                <ul class="rd " style="display:none;">
                    <li>
    ...
    <div class="m-ck-module">
        ...
        <ul id="payment_mode" class="rd">
            <li>
                <label>
                <input name="paymode" value="1" type="radio" checked>在线支付</label>
...
<script>
        ...
        $(document).ready(function(){
            ...
            if (num > totalscore) {
                $(this).val(totalscore);
                floatNotify.simple('您填写的积分超过了您的可用积分');
                return false;
            }
        ...
        $("#sub_order").click(function(){
            ...
            if($.trim(userName) == ""){
                floatNotify.simple('请填写姓名');
                return false;
            }
            var userPhone = $("#tel").val()
            if ($.trim(userPhone) == "") {
                floatNotify.simple('请填写您的手机号码');
                return false;
            }
            ...
            $.get("<?php echo U('Wap/Store/ordersave');?>", data, function(response){
                if (response.error_code == false) {
                    ...
                    floatNotify.simple(response.msg);
