...
<link href="/Public/Wap/css/guajiang/activity-style.css" rel="stylesheet" type="text/css">
<style type="text/css">
#txt {
	color: #000000;
}
...
<script src="/Public/Wap/css/guajiang/alert.js" type="text/javascript"></script>
...
<?php else: ?> <!--有优惠券的情况下-->
	...
	<?php else: ?><!---->
		...
		<?php if($Coupon['end'] == 3&&$Coupon['phone']){ ?>
			<div class="main">
				<div class="cover">
					<img src="<?php if(!$lottery['zjpic']){ ?>/Public/Wap/css/guajiang/images/activity-coupon-winning.jpg<?php }else{ echo ($lottery["zjpic"]); } ?>" />
				</div>
				<div class="boxcontent boxwhite">
						<div class="box">
...
$("#save-btn").bind("click",
	function() {
		...
		$.post("<?php echo U('Wap/Coupon/add');?>", submitData,
			function(data) {
...
function () {
	...
	$.post("<?php echo U('Wap/Coupon/exchange');?>", submitData,
	function (data) {
		if (data.success == true) {
			alert(data.msg);
...
window.shareData = {  
            ...
            "sendFriendLink": "<?php echo ($cfg_siteUrl); echo U('Coupon/index',array('token'=>$token,'id'=>$Coupon['id']));?>",
