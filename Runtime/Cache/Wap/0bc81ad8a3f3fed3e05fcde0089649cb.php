...
<script src="/Public/Wap/css/style/js/jquery.min.js" type="text/javascript"></script>
<script type="text/javascript" src="/Public/Wap/css/product/js/main.js"></script>

<link rel="stylesheet" type="text/css" media="all" href="/Public/Wap/css/date/jsDatePick.min.css" />
<script type="text/javascript" src="/Public/Wap/css/date/jsDatePick.js"></script>
...
<td><?php if($item['inputtype'] == 'select'): ?><select style="line-height:35px;" id="<?php echo ($item["fieldname"]); ?>" name="<?php echo ($item["fieldname"]); ?>"  class="dropdown-select"><?php echo ($item["optionStr"]); ?></select>
    			...
    			<textarea class="pxtextarea" style=" height:99px;overflow-y:visible" placeholder="请输入<?php echo ($item["displayname"]); ?>" id="<?php echo ($item["fieldname"]); ?>" name="<?php echo ($item["fieldname"]); ?>"></textarea>
    			<?php elseif($item['inputtype'] == 'date'): ?>
    			<input type="datetime" id="dateField" name="<?php echo ($item["fieldname"]); ?>" />
...
window.shareData = {  
            ...
            "sendFriendLink": "<?php echo ($cfg_siteUrl); echo U('Selfform/index',array('token'=>$token,'id'=>$thisForm['id']));?>",
