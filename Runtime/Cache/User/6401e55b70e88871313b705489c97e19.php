...
<li class="<?php if(ACTION_NAME == 'caronwers'): ?>current<?php endif; ?> tabli" id="tab8"><a href="<?php echo U('Car/caronwers',array('token'=>$token));?>">车主</a></li>
    ...
    <div >
        <a href="<?php echo U('Car/add_carmodel',array('token'=>$token));?>" class="btn btn-primary">
        <i class="mdi-content-add"></i>添加车型
        </a>
    </div>
            ...
            <tbody>
                <?php if(is_array($carmodel)): $i = 0; $__LIST__ = $carmodel;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td class="input-medium">
                        <a class="btn btn-warning btn-sm" href="<?php echo U('Car/edit_carmodel',array('token'=>$token,'id'=>$vo['id']));?>" title="编辑"><i class="mdi-editor-mode-edit"></i>编辑</a>
                    <a class="btn btn-warning btn-sm" href="<?php echo U('Car/del_carmodel',array('token'=>$token,'id'=>$vo['id']));?>" title="删除" onclick="return(confirm('确定要删除吗？'))" ><i class="mdi-action-delete"></i>删除</a></td>
