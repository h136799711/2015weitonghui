...
<li class="<?php if($tabid == 3): ?>current<?php endif; ?> tabli" id="tab2"><a href="<?php echo U('Groupon/config',array('token'=>$token));?>">团购配置</a></li>
    ...
    <!--tab end-->
    <div >
        <div class="pageNavigator left"> <a href="<?php echo U('Groupon/add',array('token'=>$token));?>" title="添加" class="btn btn-primary">
        <i class="mdi-content-add"></i>添加团购</a></div>
        <div class="clearfix"></div>
    </div>
    <div class="">
        <form method="post" action="" id="info">
            ...
            <table class="table table-condensed table-bordered table-striped" border="0" cellspacing="0" cellpadding="0" width="100%">
                ...
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td class="norightborder"><a class="btn btn-warning btn-sm" href="<?php echo U('Groupon/set',array('id'=>$hostlist['id'],'token'=>$token));?>"><i class="mdi-editor-mode-edit"></i>修改</a> <a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Groupon/del',array('id'=>$hostlist['id'],'token'=>$token));?>');" class="btn btn-warning btn-sm"><i class="mdi-action-delete"></i>删除</a></td>
