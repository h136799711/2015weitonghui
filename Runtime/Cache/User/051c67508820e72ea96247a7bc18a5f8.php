...
<div class="col-xs-9 col-sm-9  col-md-10 col-lg-10 ">
    <div class="righttitle">
        <h4 >团购订单管理（<a href="<?php echo U('Groupon/index',array('token'=>$token,'handled'=>0));?>">未处理订单<span style="color:#f00"><?php echo ($unhandledCount); ?></span>个</a>） (<?php echo ($page); ?>) </h4>
        <div class="searchbar right hide">
            ...
            <input type="submit" value="搜索" id="msgSearchBtn" onclick="location.href='<?php echo U('User/Groupon/index',array('token'=>$token));?>?code='+$('#msgSearchInput').val()" class="btn btn-default " title="搜索">
            <script>
...
<li class="<?php if($tabid == 3): ?>current<?php endif; ?> tabli" id="tab2"><a href="<?php echo U('Groupon/config',array('token'=>$token));?>">团购配置</a></li>
        ...
        <form method="post" action="" id="info">
            ...
            <table class="table table-condensed table-bordered table-striped" border="0" cellspacing="0" cellpadding="0" width="100%">
                ...
                <?php if(is_array($orders)): $i = 0; $__LIST__ = $orders;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$o): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td class="norightborder"><a class="btn btn-warning btn-sm" href="###" onclick="showIntroDetail(<?php echo ($o["id"]); ?>)"><i class="mdi-action-visibility"></i>详细</a> <a class="btn btn-warning btn-sm" href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Product/deleteOrder',array('token'=>$token,'id'=>$o['id'],'dining'=>$isDining));?>');"><i class="mdi-action-delete"></i>删除</a></td>
    ...
    art.dialog.open('<?php echo U('Product/orderInfo',array('token'=>$token,'dining'=>$isDining,'id'=>''));?>'+id,{lock:false,title:'订单详情',width:700,height:420,resize:true,yesText:'关闭',background: '#000',opacity: 0.87});
    }
    </script>
</div>
<div >
