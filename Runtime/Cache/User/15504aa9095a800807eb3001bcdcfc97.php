...
<div class="righttitle">
    <h4>编辑刮刮卡活动开始内容</h4><a href="javascript:history.go(-1);"  class="btn btn-primary" data-top ><i class="mdi-content-reply"></i>返回</a>
</div> 
<div class=" bgfc"> 
...
<tr>
  ...
  <td>
	<input type="input" class="form-control" id="keyword" value="<?php if($vo['keyword'] == ''): ?>刮刮卡<?php else: echo ($vo["keyword"]); endif; ?>" name="keyword" style="width:400px" ><br />
  	<span class="red">只能写一个关键词</span>，用户输入此关键词将会触发此活动。
  </td>
  <td rowspan="7" valign="top">
	  <div style="margin-left:20px">
		<img id="pic1_src" src="<?php if($vo['starpicurl'] == ''): echo ($cfg_siteUrl); ?>/Public/User/images/img/activity-scratch-card-start.jpg<?php else: echo ($vo["starpicurl"]); endif; ?>" width="373px" >	
		<br />
		<input class="form-control" id="pic1" name="starpicurl" value="<?php if($vo['starpicurl'] == ''): echo ($cfg_siteUrl); ?>/Public/User/images/img/activity-scratch-card-start.jpg<?php else: echo ($vo["starpicurl"]); endif; ?>"   onclick="document.getElementById('pic1_src').src=this.value;" style="width:363px;"  />
		<br /><input name="type" value="2" type="hidden"   />
		 <script src="/Public/static/gfile.js"></script><a href="###" onclick="gfilePicUpload('pic1',1000,500,'<?php echo ($token); ?>')" class="btn btn-warning btn-sm"><i class="mdi-file-file-upload"></i>上传</a> <a href="###" onclick="viewImg('pic1')" class="btn btn-warning btn-sm"><i class="mdi-action-pageview"></i>预览</a>&nbsp;活动开始图片
	  </div>
  </td>
</tr>
<tr>
  ...
  <td>
	<input type="input" class="form-control" id="title" value="<?php if($vo['title'] == ''): ?>刮刮卡活动开始了<?php else: echo ($vo["title"]); endif; ?>" name="title" style="width:400px" />
  	<br />
  	请不要多于50字!
  </td>
  <tr>
 ...
 <tr>
  	...
  	<td><textarea class="form-control"   name="sttxt" style="width:400px"  ><?php echo ($vo["sttxt"]); ?></textarea>
  		 </td>
</tr>
<tr>
	<th><span class="red">*</span>活动时间：</th>
	<td><input type="input" class="form-control" id="statdate" value="<?php if($vo['statdate'] != ''): echo (date("Y-m-d",$vo["statdate"])); else: echo date('Y-m-d',mktime(0, 0, 0, date("m") , date("d"), date("Y"))); endif; ?>" onClick="WdatePicker()" name="statdate" />                
		到
		<input type="input" class="form-control" id="enddate" value="<?php if($vo['enddate'] != ''): echo (date("Y-m-d",$vo["enddate"])); else: echo date('Y-m-d',mktime(0, 0, 0, date("m") , date("d")+3, date("Y"))); endif; ?>" name="enddate" onClick="WdatePicker()"  /> 
	</td>
</tr>
<tr>
<th valign="top">活动说明：</th>
<td><textarea  class="form-control" id="info" name="info"  style="width:400px; height:125px" ><?php if($vo['info'] == ''): ?>亲，请点击进入刮刮卡抽奖活动页面，祝您好运哦！<?php else: echo ($vo["info"]); endif; ?></textarea><br />换行请输入
 &lt;br&gt;
 </td>
</tr>
</tbody>
  	...
  	<table style=" margin: 0;" border="0" cellspacing="0" cellpadding="0" width="100%">
  		<tbody>
  			<tr>
  				...
  				<td><input type="input" class="form-control" id="endtite" value="<?php if($vo['endtite'] == ''): ?>刮刮卡活动已经结束了<?php else: echo ($vo["endtite"]); endif; ?>" name="endtite" style="width:400px" />
  					<br />
  					请不要多于50字! </td>
  				<td rowspan="4" valign="top"><div style="margin-left:20px"><img  id="pic2_src" src="<?php if($vo['endpicurl'] == ''): echo ($cfg_siteUrl); ?>/Public/User/images/img/activity-scratch-card-end.jpg<?php else: echo ($vo["endpicurl"]); endif; ?>"  width="373px"/> <br />
  					<input class="form-control" id="pic2" name="endpicurl" value="<?php if($vo['endpicurl'] == ''): echo ($cfg_siteUrl); ?>/Public/User/images/img/activity-scratch-card-end.jpg<?php else: echo ($vo["endpicurl"]); endif; ?>"  onchange="document.getElementById('pic2_src').src=this.value;"  style="width:363px;"  />
  					<br />
  					 <script src="/Public/static/gfile.js"></script><a href="###" onclick="gfilePicUpload('pic2',1000,500,'<?php echo ($token); ?>')" class="btn btn-warning btn-sm"><i class="mdi-file-file-upload"></i>上传</a> <a href="###" onclick="viewImg('pic2')" class="btn btn-warning btn-sm"><i class="mdi-action-pageview"></i>预览</a>&nbsp;活动结束图片</div></td>
  				</tr>
  			<tr>
...
<tr>
<th>&nbsp;</th>
<td><button type="submit" class="btn btn-primary" ><i class="mdi-content-save"></i>保存</button>　<a href=""  class="btn btn-primary"><i class="mdi-navigation-cancel"></i>取消</a>　<span class="red">请确认已开启刮刮卡功能</span></td>
</tbody>
</table>
  </div> 
 </div>
