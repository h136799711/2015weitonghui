...
<li class="<?php if(ACTION_NAME == 'aboutus'): ?>current<?php endif; ?> tabli" id="tab7"><a href="<?php echo U('Estate/aboutus',array('token'=>$token));?>">关于我们</a></li>
    ...
    <div >
        ...
        <i class="mdi-content-add"></i>添加相册
        </a>
    </div>
            ...
            <tbody>
                ...
                <?php if(is_array($album)): $i = 0; $__LIST__ = $album;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td style="text-align:center;">
                        <a href="<?php echo U('Estate/album_add',array('poid'=>$list['id'],'token'=>$list['token']));?>" class="btn btn-warning btn-sm"><i class="mdi-editor-mode-edit"></i>编辑</a>
                        <a href="<?php echo U('Estate/album_del',array('id'=>$list['mid'],'token'=>$list['token']));?>" onclick="return(confirm('确定要删除吗？'))" class="btn btn-warning btn-sm"><i class="mdi-action-delete"></i>删除</a></a>
                    </td>
