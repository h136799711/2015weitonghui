...
<li class="<?php if(ACTION_NAME == 'introduction'): ?>current<?php endif; ?> tabli" id="tab7"><a href="<?php echo U('School/introduction',array('token'=>$token));?>">菜单/回复配置</a></li>
    ...
    <div class="righttitle">
        <h4 >当前位置：微学校 >> 食谱管理</h4><a href="<?php echo U('School/cookbook_add',array('token'=>$token,'type'=>'assess'));?>" class="btn btn-primary" style="margin-top:-2px">新增食谱</a>
                ...
                <table class="table table-condensed table-bordered table-striped" border="0" cellspacing="0" cellpadding="0" width="100%">
                    ...
                    <?php if(is_array($recipe)): $i = 0; $__LIST__ = $recipe;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tbody><tr>
                        ...
                        <td class="norightborder">
                            <a href="<?php echo U('School/cookbook_add',array('id'=>$vo['id'],'type'=>$vo['type']));?>" class="btn btn-warning btn-sm"><i class="mdi-editor-mode-edit"></i>编辑</a>
                        <a href="<?php echo U('School/cookbook_del',array('id'=>$vo['id'],'type'=>$vo['type']));?>" onclick="return(confirm('确定要删除吗？'))" class="btn btn-warning btn-sm"><i class="mdi-action-delete"></i>删除</a> </td>
