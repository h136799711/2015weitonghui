                ...
                <tbody>
                    <?php if(is_array($onwers)): $i = 0; $__LIST__ = $onwers;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td class="input-medium">
                            <a class="btn btn-warning btn-sm" href="###" onclick="showIntroDetail(<?php echo ($vo["id"]); ?>)"><i class="mdi-image-details"></i>费用详细</a>
                            &nbsp;&nbsp;&nbsp;&nbsp;
                        <a class="btn btn-warning btn-sm" href="<?php echo U('Car/del_carowner',array('token'=>$token,'id'=>$vo['id']));?>" title="删除" onclick="return(confirm('确定要删除吗？'))" ><i class="mdi-action-delete"></i>删除</a></td>
    ...
    <div >
        ...
        <script>
                    function showIntroDetail(id){
                        art.dialog.open('<?php echo U('Car/owner_uinfo',array('token'=>$token,'id'=>''));?>'+id,{lock:false,title:'车主详情',width:700,height:520,resize:true,yesText:'关闭',background: '#000',opacity: 0.87});
                    }
        </script>
        <div class="clearfix"></div>
    </div>
