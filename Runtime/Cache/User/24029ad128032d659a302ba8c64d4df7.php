...
<li class="<?php if(ACTION_NAME == 'introduction'): ?>current<?php endif; ?> tabli" id="tab7"><a href="<?php echo U('School/introduction',array('token'=>$token));?>">菜单/回复配置</a></li>
          ...
          <div class="">
           <div class="bg-warning tip">注意:这里您可以添加预约的课程,供家长和学生选择. &nbsp;&nbsp;&nbsp; <a  class="btn btn-warning btn-sm" href="javascript:location.reload()">刷新本页面</a></div>
        </div>
        ...
        <a href="<?php echo U('School/subscribe_add',array('token'=>$token,'addtype'=>'course'));?>" class="btn btn-primary">
                    <i class="mdi-content-add"></i>添加预约课程
                </a>
            ...
            <table class="table table-condensed table-bordered table-striped" border="0" cellspacing="0" cellpadding="0" width="100%">
              ...
              <tbody>
                <?php if(is_array($reslist)): $i = 0; $__LIST__ = $reslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td >
                             <a href="<?php echo U('School/res_manage',array('token'=>$vo['token'],'id'=>$vo['id'],'addtype'=>$vo['addtype']));?>"  class="btn btn-warning btn-sm" title="订单管理">订单管理</a>
                            <a  class="btn btn-warning btn-sm" href="<?php echo U('School/subscribe_add',array('token'=>$vo['token'],'id'=>$vo['id'],'addtype'=>$vo['addtype'],'type'=>'edit'));?>" title="编辑"><i class="mdi-editor-mode-edit"></i>编辑</a>
                             <a  class="btn btn-warning btn-sm" href="<?php echo U('School/subscribe_del',array('token'=>$vo['token'],'id'=>$vo['id'],'addtype'=>$vo['addtype']));?>" onclick="return(confirm('确定要删除吗？'))"  title="删除"><i class="mdi-action-delete"></i>删除</a></td>
