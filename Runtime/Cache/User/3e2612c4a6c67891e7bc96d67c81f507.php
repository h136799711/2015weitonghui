...
<li class="<?php if(ACTION_NAME == 'caronwers'): ?>current<?php endif; ?> tabli" id="tab8"><a href="<?php echo U('Car/caronwers',array('token'=>$token));?>">车主</a></li>
    ...
    <div class="alert">
        <p><span class="bold">使用方法：</span>请先在回复设置里添加。 开启状态可以在手机显示，关闭就不显示</p>
    </div>
    <div class="">
        <table class="table table-condensed table-bordered table-striped" border="0" cellspacing="0" cellpadding="0" width="100%">
            ...
            <tbody>
                <?php if(is_array($utility)): $i = 0; $__LIST__ = $utility;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><form method="post" action="">
                    <tr>
                        <td><span class="sn weather"><?php echo ($vo['name']); ?></span></td>
                        <td>
                            <div class="switch switch-small">
                                <?php if($vo['status'] == 1): ?><a href="<?php echo U('Car/utility',array('token'=>$token,'status'=>'on','id'=>$vo['id'],'isPost'=>1));?>"><strong>开启</strong></a>
                                <?php else: ?>
                                <a href="<?php echo U('Car/utility',array('token'=>$token,'status'=>'off','id'=>$vo['id'],'isPost'=>1));?>"><strong>关闭</strong></a><?php endif; ?>
                            </div>
                        </td>
                    </tr>
