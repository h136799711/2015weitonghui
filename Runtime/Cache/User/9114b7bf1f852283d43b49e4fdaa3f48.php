...
<li class="<?php if(ACTION_NAME == 'introduction'): ?>current<?php endif; ?> tabli" id="tab7"><a href="<?php echo U('School/introduction',array('token'=>$token));?>">菜单/回复配置</a></li>
    ...
    <div class="righttitle">
        <h4 >当前位置：微学校 >> 学员管理</h4><a href="<?php echo U('School/student_add',array('token'=>$token,'type'=>'semester'));?>" class="btn btn-primary" style="margin-top:-2px">新增学员</a>
    ...
    <div class="cLineD">
        <form method="post" id="formID" action="<?php echo U('School/search',array('token'=>$token));?>">
            ...
            姓名:<input size="20" value=""  placeholder="请输入学员姓名"  name="searchkey[name]" class="form-control">&nbsp;&nbsp;&nbsp;手机号:<input size="20" value="" name="searchkey[phone]"placeholder="请输入学员电话" class="form-control">&nbsp;&nbsp;&nbsp;
            <button type="submit" name="appidsubmit"  value="true" class="btn btn-warning btn-sm" ><strong>查询</strong></button>
        </form>
    </div>
    <div class="">
        <div class="bg-warning tip">注意:地址只显示街道地址,上面查询支持模糊查询.</div>
        ...
        <table class="table table-condensed table-bordered table-striped" border="0" cellSpacing="0" cellPadding="0" width="100%">
            ...
            <?php if(is_array($students)): $i = 0; $__LIST__ = $students;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td>
                    <a class="btn btn-warning btn-sm" href="<?php echo U('School/student_add',array('token'=>$token,'id'=>$list['id'],'type'=>'edit'));?>"><i class="mdi-editor-mode-edit"></i>编辑</a>
                    <a class="btn btn-warning btn-sm" href="<?php echo U('School/del_item',array('token'=>$vo['token'],'id'=>$list['id'],'deltype'=>'students'));?>" onclick="return(confirm('确定要删除吗？'))"   title="删除"><i class="mdi-action-delete"></i>删除</a>
                    <a class="btn btn-warning btn-sm" href="<?php echo U('School/student_score',array('token'=>$token,'id'=>$list['id']));?>"><i class="mdi-editor-border-color"></i>录成绩</a>
                </td>
