...
<div class="col-xs-9 col-sm-9  col-md-10 col-lg-10 ">
 ...
 <ul class="nav nav-pills">
    ...
    <div >
        <a href='<?php echo U("Voiceresponse/add");?>' title='添加语音回复' class='btn btn-primary'><i class="mdi-content-add"></i>添加语音回复</a>
    </div>
    <div class="">
        <table class="table table-condensed table-bordered table-striped" border="0" cellSpacing="0" cellPadding="0" width="100%">
            ...
            <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                <td>
                  <?php echo ($key+1); ?>
                  <!--   <input type="checkbox" name="del_id[]" value="<?php echo ($vo["id"]); ?>" class="checkitem"> --></td>
                  
                <td><?php echo ($vo["keyword"]); ?></td>
                <td><?php echo ($vo["title"]); ?></td>
                <td>
                    <?php if($vo["musicurl"] != ''): ?><a style="width:120px;float:left" id="musicurl_src" class="audio {skin:'blue'}" href="<?php echo ($vo["musicurl"]); ?>">音乐播放</a><?php endif; ?>
                </td>

                <td><?php echo date('Y-m-d',$vo['uptatetime']); ?></td>
                <td class="norightborder"> <a class="btn btn-warning btn-sm"  href="<?php echo U('Voiceresponse/edit',array('id'=>$vo['id']));?>" title="编辑图文自定义回复"><i class="mdi-editor-mode-edit"></i>编辑</a>  <a href="javascript:drop_confirm('您确定要删除吗?', \'<?php echo U('Voiceresponse/del',array('id'=>$vo['id']));?>\');" class="btn btn-warning btn-sm" ><i class="mdi-action-delete"></i>删除</a></td>
