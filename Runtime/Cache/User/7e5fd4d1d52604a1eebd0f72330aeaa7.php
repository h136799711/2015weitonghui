...
<div class="col-xs-9 col-sm-9  col-md-10 col-lg-10 ">
    <div class="righttitle"><h4>3G站点底部菜单设置</h4></div>

    <ul id="tags" class="nav nav-tabs">
        <li class="active" role="presentation"><a href="<?php echo U('Catemenu/index');?>">底部菜单分类设置</a> </li>
    ...
    <div class="bg-warning tip" style="margin:10px 0 0 0">一级主菜单最多4个，菜单风格1-8无子菜单，菜单风格9-16子菜单最多10个。</div>
    <div class="cLineC" style="line-height: 32px;">
        ...
        <?php }};?>
        class="btn btn-primary">添加<?php if($fid != 0): echo ($thisCatemanu["name"]); ?>子<?php endif; ?>菜单</a>
    </div>
    <div class=" form">
                ...
                <table class="table table-condensed table-bordered table-striped" border="0" cellSpacing="0" cellPadding="0" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th style=" width:220px;" class="norightborder">操作</th>
                        </tr>
                    </thead>
                    <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        <td><?php echo ($vo["name"]); ?></td>
                        <td><?php echo ($vo["orderss"]); ?></td>
                        <td><img src="<?php echo ($vo["picurl"]); ?>" class="thumbnail"  /></td>
                        <!-- <td><?php echo ($vo["url"]); ?></td> -->
                        <td><?php if(($vo["status"]) == "1"): ?>显示<?php else: ?>不显示<?php endif; ?></td>
                        <td class="norightborder">
                            <?php if ($vo['fid']==0){?><a class="btn btn-warning btn-sm " href="<?php echo U('Catemenu/index',array('fid'=>$vo['id']));?>"><i class="mdi-content-add-circle"></i>子菜单</a><?php }?>
                            <a class="btn btn-warning btn-sm " href="<?php if ($vo['fid']==0){ echo U('Catemenu/edit',array('id'=>$vo['id'])); }else{ echo U('Catemenu/edit',array('id'=>$vo['id'],'fid'=>$vo['fid'])); }?>"><i class="mdi-editor-mode-edit"></i>编辑</a>
                            <a class="btn btn-warning btn-sm " href="<?php if ($vo['fid']==0){ echo U('Catemenu/del',array('id'=>$vo['id'])); }else{ echo U('Catemenu/del',array('id'=>$vo['id'],'fid'=>$vo['fid'])); }?>"><i class="mdi-action-delete"></i>删除</a>
                        </td>
