...
<?php if($ischild != 1): ?><li class="tabli" id="tab5"><a href="<?php echo U('ReplyInfo/set', array('token'=>$token, 'infotype' => 'Hotels'));?>">酒店预订回复配置</a></li><?php endif; ?>
</ul>
</div>
<!--tab end-->
<div >
<div class="pageNavigator left"> <a href="<?php echo U('Hotels/add',array('token' => $token,'cid'=>$cid));?>" title="添加" class="btn btn-primary"><i class="mdi-content-add"></i>添加房间类型</a></div>
<div class="clearfix"></div>
</div>
<div class="">
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tablelist): $mod = ($i % 2 );++$i;?><tr>
		...
		<td class="norightborder">
			<a class="btn btn-warning btn-sm" href="<?php echo U('Hotels/add', array('id' => $tablelist['id'], 'token' => $token,'cid' => $tablelist['cid']));?>"><i class="mdi-editor-mode-edit"></i>修改</a> 
			<a class="btn btn-warning btn-sm"  href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Hotels/del',array('id' => $tablelist['id'], 'token' => $token,'cid' => $tablelist['cid']));?>');"><i class="mdi-action-delete"></i>删除</a>
		</td>
