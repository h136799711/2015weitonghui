...
<li class="<?php if(ACTION_NAME == 'aboutus'): ?>current<?php endif; ?> tabli" id="tab7"><a href="<?php echo U('Estate/aboutus',array('token'=>$token));?>">关于我们</a></li>
          ...
          <div >
    <div class="alert">
    <p><span class="bold">使用方法：添加一项即可,请勿重复添加。</span> <a class="btn btn-warning btn-sm" href="javascript:location.reload()"><i class="mdi-navigation-refresh"></i>刷新</a></p>
    </div>
</div>
          ...
          <?php if($count == 0): ?><a href="<?php echo U('Estate/reservation_add',array('token'=>$token));?>" class="btn btn-primary">
                    <i class="mdi-content-add"></i>添加预约
                </a><?php endif; ?>
            </div>
            ...
            <table class="table table-condensed table-bordered table-striped" border="0" cellspacing="0" cellpadding="0" width="100%">
              ...
              <tbody>
                <?php if(is_array($reslist)): $i = 0; $__LIST__ = $reslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td >
                             <a href="<?php echo U('Estate/reservation_manage',array('token'=>$vo['token'],'id'=>$vo['id'],'addtype'=>$vo['addtype']));?>" class="btn btn-warning btn-sm" title="订单管理">订单管理</a>
                            <a class="btn btn-warning btn-sm" href="<?php echo U('Estate/reservation_edit',array('token'=>$vo['token'],'id'=>$vo['id'],'addtype'=>$vo['addtype']));?>" title="编辑"><i class="mdi-editor-mode-edit"></i>编辑</a> 
                             <a class="btn btn-warning btn-sm" href="<?php echo U('Estate/reservation_del',array('token'=>$vo['token'],'id'=>$vo['id'],'addtype'=>$vo['addtype']));?>" onclick="return(confirm('确定要删除吗？'))"  title="删除"><i class="mdi-action-delete"></i>删除</a></td>
