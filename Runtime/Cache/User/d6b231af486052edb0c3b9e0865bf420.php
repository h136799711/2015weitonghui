...
<li class="<?php if(ACTION_NAME == 'introduction'): ?>current<?php endif; ?> tabli" id="tab7"><a href="<?php echo U('School/introduction',array('token'=>$token));?>">菜单/回复配置</a></li>
          ...
          <div class="righttitle">
              <h4 >当前位置：微学校 >> 教师管理</h4><a href="<?php echo U('School/assess_add',array('token'=>$token,'type'=>'assess'));?>" class="btn btn-primary" style="margin-top:-2px">新增教师</a>
              <div class="clearfix"></div>
          </div>
          <div class="cLineD">
    <form method="post" action="<?php echo U('School/search',array('token'=>$token));?>">
    <input type="hidden" name="searchkey[key]" value="assess">
        姓名:<input size="20" value="" name="searchkey[name]" class="form-control">&nbsp;&nbsp;&nbsp;入职时间:<input size="20" value="" name="searchkey[time]" class="form-control" >&nbsp;&nbsp;&nbsp;手机号:<input size="20" value="" name="searchkey[phone]" class="form-control">&nbsp;&nbsp;&nbsp;电子邮件:<input size="20" value="" name="searchkey[email]" class="form-control">&nbsp;&nbsp;&nbsp;
        <button type="submit" name="appidsubmit" value="true" class="btn btn-warning btn-sm" ><strong>查询</strong></button>
    </form>
 </div>
          <div class="">
           <div class="bg-warning tip">注意:支持模糊查询</div>
...
<!--
                 ...
                 <td> </td> -->
                 <td>
<a class="btn btn-warning btn-sm" href="<?php echo U('School/assess_add',array('token'=>$token,'tid'=>$list['tid'],'type'=>'edit'));?>" title="编辑"><i class="mdi-editor-mode-edit"></i>编辑</a>
<a class="btn btn-warning btn-sm" href="<?php echo U('School/del_item',array('token'=>$list['token'],'id'=>$list['tid'],'deltype'=>'assess'));?>" onclick="return(confirm('确定要删除吗？'))"  title="删除"><i class="mdi-action-delete"></i>删除</a>
<a class="btn btn-warning btn-sm" href="<?php echo U('School/assess_course',array('token'=>$token,'tid'=>$list['tid']));?>" title="添加课程" >添加课程</a>
                 </td>
