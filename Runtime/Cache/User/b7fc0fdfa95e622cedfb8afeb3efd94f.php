...
<li class="<?php if(ACTION_NAME == 'orders' OR (ACTION_NAME == 'orders_list')): ?>current<?php endif; ?> tabli" id="tab2"><a href="<?php echo U('Business/orders',array('token'=>$token,'type'=>'beauty'));?>">订单管理</a></li>
 ...
 <div class="cLineC" style="line-height: 32px;"> <a href="<?php echo U('Business/classify_add',array('token'=>$token,'type'=>$type));?>" class="btn btn-primary">
 添加
...
美容公司<?php endif; ?>
          ...
          <?php if(is_array($busines_main)): $i = 0; $__LIST__ = $busines_main;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td>
                     <a href="<?php echo U('Business/classify_add',array('mid'=>$vo['mid'],'type'=>$vo['type']));?>" class="btn btn-warning btn-sm" ><i class="mdi-editor-mode-edit"></i>编辑</a>
              <a href="<?php echo U('Business/classify_del',array('mid'=>$vo['mid'],'type'=>$vo['type']));?>" onclick="return(confirm('确定要删除吗？'))" class="btn btn-warning btn-sm" ><i class="mdi-action-delete"></i>删除</a>
                    </td>
