...
class GrouponController extends ProductController{
	public $token;
	public $wecha_id;
	...
	public function _initialize(){
		...
		if(empty($grouponConfig)){
			$groupon = M('Reply_info')->where(array('infotype'=>$infotype,'token'=>$this->token))->find();
				addWeixinLog($groupon,'团购配置');
			if($groupon){
				$grouponConfig = $groupon;
				addWeixinLog($grouponConfig,'团购配置');
			}
		}
		$grouponDetailConfig=unserialize($grouponConfig['config']);
		$this->tplid=intval($grouponDetailConfig['tplid']);
		$this->assign('pageSize',$this->pageSize);
		$this->assign('wecha_id',I('get.wecha_id'));
	...
	public function grouponIndex(){
		...
		if (IS_POST){
			$key = I('post.search_name');
            		...
            		$this->redirect('Wap/Groupon/grouponIndex',array('keyword'=>$key,'token'=>$this->token));
		}
	...
	public function my(){
		...
		$alipayConfig=$AlipayConfig_db->where(array('token'=>$this->token))->find();
	...
	public function orderCart(){
		...
		if (IS_POST){
			...
			$row['orderid']=$orderid;
			//
			$row['truename']=I('post.truename');
			$row['tel']=I('post.tel');
			$row['address']=I('post.address');
			...
			if ($alipayConfig['open']){
				...
				$this->redirect('Groupon/my',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'],'success'=>1));
			}
		}else {
			$where=array('token'=>$this->token);
			if (isset($_GET['id'])){
	...
	public function handle(){
		...
		if (IS_POST){
			...
			$thisStaff=$staff_db->where(array('username'=>I('post.username'),'token'=>I('get.token')))->find();
			if (!$thisStaff){
	...
	public function payReturn(){
		...
		if($order){
			...
			$this->redirect('Wap/Groupon/myOrders',array('wecha_id'=>$order['wecha_id'],'token'=>$order['token']));
		}else{
