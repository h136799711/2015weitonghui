...
class EstateController extends BaseController{
    ...
    public function album(){
        ...
        $album = $t_album->where(array('token'=>I('get.token')))->field('id,poid')->select();
        $list_photo = array();
        foreach ($album as $k => $v) {
             $list_photo[] = $Photo->where(array('token'=>I('get.token'),'id'=>$v['poid']))->find();
        }
    ...
    public function show_album(){
        $t_housetype = M('Estate_housetype');
        $id = (int)I('get.id');
        $where = array('token'=>I('get.token'),'id'=>$id);
