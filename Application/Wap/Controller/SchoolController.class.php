...
class SchoolController extends  WapController{
    ...
    public function readview(){
        ...
        $token      = filter_var(I('get.token'),FILTER_SANITIZE_STRING);
        $readtype   = filter_var(I('get.readtype'),FILTER_SANITIZE_STRING);
    ...
    public function qresults(){
       $wecha_id  = trim(filter_var(I('get.wecha_id'),FILTER_SANITIZE_STRING));
       $t_student = M('school_students');
       //check wecha_id is ok ,display to this children results.
       if(IS_POST){
          $s_name = trim(filter_var(I('post.s_name'),FILTER_SANITIZE_STRING));
          $mobile = trim(filter_var(I('post.mobile'),FILTER_SANITIZE_STRING));
          $token  = trim(filter_var(I('post.token'),FILTER_SANITIZE_STRING));
    ...
    public function yysave(){
       ...
       if(IS_POST){
          $t_book             = M('reservebook');
          $_POST['wecha_id']  = trim(filter_var(I('post.wecha_id'),FILTER_SANITIZE_STRING));
          $_POST['token']     = trim(filter_var(I('post.token'),FILTER_SANITIZE_STRING));
          $_POST['truename']  = trim(filter_var(I('post.truename'),FILTER_SANITIZE_STRING));
          $_POST['tel']       = trim(filter_var(I('post.mobile'),FILTER_SANITIZE_STRING));
          $_POST['housetype'] = trim(filter_var(I('post.housetype'),FILTER_SANITIZE_STRING));
          $_POST['info']      = trim(filter_var(I('post.info'),FILTER_SANITIZE_STRING));
          $_POST['type']      = trim(filter_var(I('post.type'),FILTER_SANITIZE_STRING));
          $_POST['dateline']  = trim(filter_var(I('post.dateline'),FILTER_SANITIZE_STRING));
          $_POST['address']   = trim(filter_var(I('post.address'),FILTER_SANITIZE_STRING));
          $_POST['choose']    = trim(filter_var(I('post.choose'),FILTER_SANITIZE_STRING));
          $_POST['booktime']  = time();
          $_POST['rid']       = trim(filter_var(I('post.rid'),FILTER_VALIDATE_INT));
    ...
    public function mylist(){
        ...
        $token     = trim(filter_var(I('get.token'),FILTER_SANITIZE_STRING));
        $wecha_id  = trim(filter_var(I('get.wecha_id'),FILTER_SANITIZE_STRING));
        $rid       = trim(filter_var(I('get.rid'),FILTER_VALIDATE_INT));
        $type      = trim(filter_var(I('get.type'),FILTER_SANITIZE_STRING));
        $check     = trim(filter_var(I('get.check'),FILTER_VALIDATE_INT));
        if(empty($check)){
    ...
    public function  del(){
        ...
        if($check){
            ...
            $this->success('删除成功',U('School/mylist',array('token'=>$token,'wecha_id'=>$wecha_id,'type'=>I('get.type'),'check'=>time())));
             exit;
         }else{
            $this->error('非法操作',U('School/mylist',array('token'=>$token,'wecha_id'=>$wecha_id,'type'=>I('get.type'),'check'=>time())));
            exit;
         }
    }
    ...
    public function sscore(){
        ...
        $token     = trim(filter_var(I('get.token'),FILTER_SANITIZE_STRING));
        $wecha_id  = trim(filter_var(I('get.wecha_id'),FILTER_SANITIZE_STRING));
        $sid       = trim(filter_var(I('get.sid'),FILTER_VALIDATE_INT));
        $check     = trim(filter_var(I('get.check'),FILTER_VALIDATE_INT));
        if(empty($check)){
