...
class TenpayController extends BaseController{
	public $token;
	public $wecha_id;
	public $payConfig;
	...
	public function pay(){
		...
		$return_url = C('site_url').U('Wap/Tenpay/return_url',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'from'=>$_GET['from']));
		...
		$notify_url = C('site_url').U('Wap/Tenpay/notify_url');
	...
	public function return_url (){
		...
		if($resHandler->isTenpaySign()) {
			...
			if( "0" == $pay_result  ) {
				...
				$this->redirect('Wap/'.$from.'/payReturn',array('token'=>$_GET['token'],'wecha_id'=>$thisOrder['wecha_id'],'orderid'=>$out_trade_no));
