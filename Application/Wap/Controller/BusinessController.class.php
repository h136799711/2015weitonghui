...
class BusinessController extends WapController{
    ...
    public function _initialize() {
        parent::_initialize();
        $this->token    = filter_var(I('get.token'),FILTER_SANITIZE_STRING);
        $this->wecha_id = filter_var(I('get.wecha_id'),FILTER_SANITIZE_STRING);
        $this->type     = filter_var(I('get.type'),FILTER_SANITIZE_STRING);
        $this->bid      = filter_var(I('get.bid'),FILTER_VALIDATE_INT);
        $arrAllow       = array('fitness','gover','food','travel','flower','property','ktv','bar','fitment','wedding','affections','housekeeper','lease','beauty');
        $orderid    =   filter_var(I('get.orderid'),FILTER_SANITIZE_STRING);
    ...
    public function index(){
        ...
        $wecha_id = filter_var(I('get.wecha_id'),FILTER_SANITIZE_STRING);
        ...
        $this->assign('busines',$busines);
        $show    = filter_var(I('get.show'),FILTER_SANITIZE_STRING);
        if($show == 'intro' && $show != ''){
...
$info[0]['url']  = U('Wap/Business/index',array('token'=>$token,'wecha_id'=>$wecha_id,'type'=>$type,'bid'=>$bid));
$info[0]['img']  = __ROOT__.'/Public/static/attachment/icon/white/1.png';

$info[1]['url']  =U('Wap/Business/index',array('token'=>$token,'wecha_id'=>$wecha_id,'type'=>$type,'bid'=>$bid,'show'=>'intro'));
$info[1]['img']  = __ROOT__.'/Public/static/attachment/icon/white/5.png';

$info[2]['url']  = U('Wap/Business/classify',array('token'=>$token,'wecha_id'=>$wecha_id,'type'=>$type,'bid'=>$bid));
$info[2]['img']  = __ROOT__.'/Public/static/attachment/icon/white/4.png';

$info[3]['url']  = U('Wap/Business/plist',array('token'=>$token,'wecha_id'=>$wecha_id,'type'=>$type,'bid'=>$bid));
$info[3]['img']  = __ROOT__.'/Public/static/attachment/icon/white/13.png';

$info[4]['url']  = U('Wap/Business/comments',array('token'=>$token,'wecha_id'=>$wecha_id,'type'=>$type,'bid'=>$bid));
$info[4]['img']  = __ROOT__.'/Public/static/attachment/icon/white/9.png';

$info[5]['url']  = U('Wap/Business/mylist',array('token'=>$token,'wecha_id'=>$wecha_id,'type'=>$type,'bid'=>$bid));
$info[5]['img']  = __ROOT__.'/Public/static/attachment/icon/white/15.png';
...
switch($busines['type']){
    ...
    public function goCart(){
        ...
        if(IS_POST){
             ...
             $_POST['bid']        = filter_var(I('post.bid'),FILTER_VALIDATE_INT);
             $_POST['sid']        = filter_var(I('post.sid'),FILTER_VALIDATE_INT);
             $_POST['token']      = filter_var(I('post.token'),FILTER_SANITIZE_STRING);
             $_POST['wecha_id']   = filter_var(I('post.wecha_id'),FILTER_SANITIZE_STRING);
             $_POST['truename']   = trim(filter_var(I('post.truename'),FILTER_SANITIZE_STRING));
             $_POST['tel']        = filter_var(I('post.tel'),FILTER_SANITIZE_STRING);
             $_POST['address']    = filter_var(I('post.address'),FILTER_SANITIZE_STRING);
             $_POST['info']       = filter_var(I('post.info'),FILTER_SANITIZE_STRING);
             $_POST['productName']= filter_var(I('post.productName'),FILTER_SANITIZE_STRING);
            ...
            $_POST['rid']         = filter_var(I('post.sid'),FILTER_VALIDATE_INT);
    ...
    protected function payReturn(){
       ...
       if($checkOrder){//如果订单存在
        ...
        // $this->redirect('Business/index', array('token'=>I('get.token'),
        //                                         'wecha_id'=>I('get.wecha_id'),'type'=>I('get.type'),'bid'=>I('get.bid'),
        //                                         'sid'=>I('get.sid')), 2, '您好,系统里没有该订单...');
          exit('订单不存在!');
        }
    ...
    public function plist(){
        ...
        if ($config){
            ...
            $headpic=__ROOT__.'/Public/Wap/default/common/css/Photo/banner.jpg';
        }
    ...
    $photo_list=M('PhotoList')->where(array('token'=>$token,'pid'=>$get_id['ablum_id'],'status'=>1))->order('sort desc')->select();
