...
class CardController extends WapController{
	...
	public function _initialize(){
		...
		$this->thisUser = M('Userinfo')->where(array('token'=>I('get.token'),'wecha_id'=>$this->wecha_id))->find();
		if (!$this->wecha_id){
	...
	public function index(){
		...
		if ($cardsCount&&isset($_GET['mycard'])){
			...
			echo '<script>location.href="'.U('Wap/Card/card',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'cardid'=>$cards[0]['cardid'])).'";</script>';
		}
    		...
    		if (!$hasApplied){
    			...
    			if(!$card){
    				...
    				if (intval($thisCard['miniscore'])==0||$userScore>intval($thisCard['miniscore'])){
    					...
    					header('Location:'.U('Wap/Userinfo/index',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'cardid'=>$thisCard['id'])));
    				}else {
    					$error=-3;
    				}
    			}
    ...
    public function signscore(){
    	...
    	$firstSecondOfMonth=mktime(0,0,$month,1,$year);
    ...
    public function action_usePrivelege(){
    	if (IS_POST){
    		...
    		$thisStaff=$staff_db->where(array('username'=>I('post.username'),'token'=>$thisCard['token']))->find();
    		if (!$thisStaff){
    			echo'{"success":-4,"msg":"用户名和密码不匹配"}';
    			exit();
    		}else {
    			if (md5(I('post.password'))!=$thisStaff['password']){
    				echo'{"success":-4,"msg":"用户名和密码不匹配"}';
    				exit();
    			}else {
    				$now=time();
    ...
    function action_useCoupon(){
    	...
    	if (IS_POST){
    		...
    		$useCount=M('Member_card_use_record')->where(array('itemid'=>$itemid,'cat'=>3,'wecha_id'=>I('post.wecha_id')))->sum('usecount');
 ...
 //    }
	...
	// 	if($get_card!=false){
	// 		Header("Location: ".C('site_url').'/'.U('Wap/Card/vip',array('token'=>I('get.token'),'wecha_id'=>I('get.wecha_id')))); 
	// 	}
	// 	if($token!=false){
	// 		//会员卡信息
	// 		$data=M('member_card_set')->where(array('token'=>$token))->find();
 ...
 //    }
	...
	// 	// 	// $contact=M('member_card_contact')->where(array('token'=>$token))->order('sort desc')->find();
	// 	// 	//我的卡号
	// 	// 	$mycard=M('member_card_create')->where(array('token'=>I('get.token'),'wecha_id'=>I('get.wecha_id')))->find();
 ...
 //    }
	...
	// 	// 	$info=M('member_card_info')->where(array('token'=>$token))->find();
	// 	// 	//卡号
	// 	// 	$card=M('member_card_create')->where(array('token'=>$token,'wecha_id'=>I('get.wecha_id')))->find();
	// 	// 	//var_dump($card);exit;
