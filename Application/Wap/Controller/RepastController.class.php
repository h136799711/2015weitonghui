...
class RepastController extends WapController{
    public $token;
    public $wecha_id = '';
    ...
    public function index(){
        ...
        if (count($company) == 1){
            $this -> redirect('Repast/select', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id, 'cid' => $company[0]['id']));
        }
    ...
    public function select(){
        ...
        if ($company = M('Company') -> where(array('token' => $this -> token, 'id' => $cid)) -> find()){
            ...
            $this -> redirect('Repast/index', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id));
        }
    ...
    public function selectTable(){
        ...
        $entity = M('address')->where(array('ucharid'=>$this -> wecha_id))->order('create_time desc')->find();
        $address = "";
    ...
    public function dish(){
        ...
        if (empty($userInfo)){
            $this -> redirect('Repast/select', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id, 'cid' => $this -> _cid));
        }
    ...
    public function saveMyOrder(){
        ...
        $address['ucharid'] = $this -> wecha_id;
        $address['detailaddress'] = $userInfo['address'];
        $address['create_time'] = time();
        if(M('Address')->where(array("ucharid"=>$this -> wecha_id,"detailaddress"=>$address))->find() === FALSE){
            M('Address')->add($address);
        }
        if ($doid){

            if ($userInfo['takeaway'] != 2){
                if ($userInfo['takeaway'] == 1){
                    $company = M('Company') -> where(array('cid' => $this -> _cid)) -> find();

                    //插入打印机
                    D('Printqueue')->InsertOrder($userInfo,$temp,$company);

                    // sendTextTofan(getToken(),$this -> wecha_id,"13484379290");
                    //发送短信提醒
            ...
            if ($alipayConfig['open'] && $dishCompany['payonline']){
                ...
                $this -> redirect('Repast/myOrder', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id, 'cid' => $this -> _cid, 'success' => 1));
            }
    ...
    public function payReturn(){
        ...
        if ($order = M('dish_order') -> where(array('orderid' => $orderid, 'token' => $this -> token)) -> find()){
            ...
            $this -> redirect('Repast/myOrder', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id, 'cid' => $this -> _cid));
        }else{
            exit('订单不存在');
        }
    }
