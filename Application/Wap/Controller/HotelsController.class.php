...
class HotelsController extends WapController{
    public $token;
    public $wecha_id = '';
    ...
    public function index(){
        ...
        if (count($company) == 1){
            $this -> redirect('Hotels/selectdate', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id, 'cid' => $company[0]['id']));
        }
    ...
    public function selectdate(){
        ...
        if ($company = M('Company') -> where(array('token' => $this -> token, 'id' => $cid)) -> find()){
            ...
            $this -> redirect('Hotels/index', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id));
        }
        $dates = array();
    ...
    public function hotel(){
        ...
        if ($days < 1){
            $this -> redirect('Hotels/selectdate', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id));
        }
            ...
            if ($dataBase -> create() !== false){
                ...
                if ($action != false){
                    ...
                    if ($alipayConfig['open']){
                        ...
                        $this -> redirect('Hotels/my', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id));
                    }
                }else{
                    $this -> error('操作失败');
                }
