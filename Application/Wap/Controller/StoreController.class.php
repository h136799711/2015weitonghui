...
class StoreController extends WapController{
	...
	public function preview() {
		...
		$listtpl = I('get.listtpl');
		$where = array('token'=> $this->token,'infotype'=>'Shop');
		...
		$this->assign('date', $info);
		$this->assign('metaTitle', '商品分类');
		if($listtpl == 0){
			$this->display('cats_2');
		}else{
			$this->display('cats_'.$listtpl);
		}

	}
	public function index() {
		$listtpl = $this->getListTpl();
		$where = array('token'=> $this->token,'infotype'=>'Shop');
		$info = M('reply_info')->where($where)->find();
	...
	public function products() {
		...
		if (IS_POST){
			$key = I('post.search_name');
			
            $this->redirect('Wap/Store/products',array('keyword'=>$key,'token'=>$this->token,'wecha_id'=>$this->wecha_id));
		}
	...
	public function product() {
		...
		if (empty($product)) {
			$this->redirect('Store/products',array('token' => $this->token,'wecha_id' => $this->wecha_id));
		}
	...
	public function deleteCart(){
		...
		$_SESSION[$this->session_cart_name] = serialize($carts);
		$this->redirect('Store/cart',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id']));
	}
	...
	public function ordersave() {
		...
		if ($carts){
			...
			$setting = M('ProductSetting')->where(array('token' => $this->token))->find();
		...
		if ($normal_rt){
			...
			$product_cart_list_model = M('ProductCart_list');
			$crow = array();
			...
			foreach ($carts as $pid => $rowset) {
				...
				if (is_array($rowset)) {
					foreach ($rowset as $did => $ro) {
						...
						M('ProductDetail')->where(array('id' => $did, 'pid' => $pid, 'num' => array('gt', $ro['count'])))->setDec('num', intval($ro['count']));
						$total += $ro['count'];
					}
				} else {
					$total = $rowset;
			...
			$alipayConfig = M('AlipayConfig')->where(array('token' => $this->token))->find();
			// if ($alipayConfig['open'] && $totalprice && $row['paymode'] == 1) {
			// 	$this->redirect('Alipay/pay',array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'success' => 1, 'from'=> 'Store', 'orderName' => $orderid, 'single_orderid' => $orderid, 'price' => $totalprice));
			// } else {
			// 	$this->redirect('Store/my',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'],'success'=>1));
			// }
			...
			exit(json_encode(array('error_code' => true, 'msg' => '订单生成失败')));
		} 
	}
	...
	public function orderCart() {
		...
		$data = $this->getCat($this->_getCart());
		if (empty($data[0])) {
			redirect(U('Store/cart', array('token' => $this->token, 'wecha_id' => $this->wecha_id)));
		}
		if (isset($data[1])) {
			foreach ($data[1] as $pid => $row) {
				$totalCount += $row['total'];
	...
	public function deleteOrder(){
		...
		redirect(U('Store/my',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'])));
	}
	...
	public function payReturn() {
	   ...
	   if ($order = M('ProductCart')->where(array('orderid' => $orderid, 'token' => $this->token))->find()) {
	   		// M('ProductCart')->where(array('orderid' => $orderid, 'token' => $this->token))->find() 
			//TODO 发货的短信提醒
			if ($order['paid']) {
				$userInfo = D('Userinfo')->where(array('token' => $this->token, 'wecha_id' => $this->wecha_id))->find();
				import("Org.Sms");
			...
			redirect(U('Store/my',array('token' => $this->token,'wecha_id' => $this->wecha_id)));
	   }else{
	      exit('订单不存在');
	    }
	}
