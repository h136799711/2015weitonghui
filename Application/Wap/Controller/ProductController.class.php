...
class ProductController extends WapController{
	...
	public function products(){
		...
		if (IS_POST){
            ...
            $this->redirect('Wap/Product/products',array('token'=>$this->token,'keyword'=>$key));
		}
		...
		if (IS_POST){
			...
			if ($cartsCount){
				...
				if (count($diningCart)){
					...
					$row['price']=$calCartInfo[1];
					//
					$row['diningtype']=intval(I('post.diningtype'));
					$row['buytime']=$buytimestamp?$row['month'].'月'.$row['day'].'日'.$row['hour'].'点':'';
					$row['tableid']=intval(I('post.tableid'));
				...
				if (intval(I('post.tableid'))&&I('post.buytimestamp')){//只是预定餐桌
					$row['total']=0;
					$row['price']=0;
					//
					$row['diningtype']=intval(I('post.diningtype'));
