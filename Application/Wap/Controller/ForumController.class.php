...
class ForumController extends WapController{
	...
	public function moreList(){
		...
		if(!empty($list2)){
			foreach($list2 as $k=>$v){
				...
				$tpl .='<div style="clear:both"></div><div><h5>'.$v['title'].'</h5><div>'.htmlspecialchars_decode($list2["$k"]["content"],ENT_QUOTES).'</div></div><a href="'.U('Wap/Forum/comment',array('tid'=>$v['id'],'token'=>$token,'wecha_id'=>$wecha_id)).'">查看全文</a></section><footer><ul class="box"><li>';
				...
				$tpl .= '</li><li><a href="'.U('Wap/Forum/comment',array('tid'=>$v['id'],'token'=>$token,'wecha_id'=>$wecha_id)).'" class="a_comment"><span class="icons icons_comment" >&nbsp;</span><label>'.$v['cnum'].'</label></a></li><li>';
	...
	public function checkAdd(){
		...
		$data['content'] = I('post.form_article');
		...
		if($forum->create()){
			if($forum->add($data)){
				if($conf['ischeck'] == 1){
					...
					$this->redirect('Forum/index',array('wecha_id'=>$data['uid'],'token'=>$data['token']));
				}
		...
		$comment = M('Forum_comment');
			...
			if($comment->add($data)){
				...
				if($conf['comcheck'] == 1){
					...
					$message['content'] = '<a href="'.U('Wap/Forum/comment',array('tid'=>$data['tid'],'token'=>$data['token'],'wecha_id'=>$uid['uid'])).'">'.$data['uname'].'评论了您的帖子,该评论需要等待管理员审核后才能显示</a>';
				}else{

					$message['content'] = '<a href="'.U('Wap/Forum/comment',array('tid'=>$data['tid'],'token'=>$data['token'],'wecha_id'=>$uid['uid'])).'">'.$data['uname'].'评论了您的帖子</a>';
				}
			...
			$message['touname'] = $userinfo['wechaname'] ? $userinfo['wechaname'] : '游客';
				...
				if($conf['comcheck'] == 1){
					...
					$this->redirect("Forum/comment",array('tid'=>$data['tid'],'wecha_id'=>$data['uid'],'token'=>$data['token']));
				}
		...
		if($comment->create()){
			...
			if($comment->add($data)){
				if($conf['comcheck'] == 1){
					...
					$message['content'] = '<a href="'.U('Wap/Forum/comment',array('tid'=>$data['tid'],'token'=>$data['token'],'wecha_id'=>$data['replyid'])).'">'.$data['uname'].'回复了您的评论，该评论在管理员审核后才能显示</a>';
				}else{

					$message['content'] = '<a href="'.U('Wap/Forum/comment',array('tid'=>$data['tid'],'token'=>$data['token'],'wecha_id'=>$data['replyid'])).'">'.$data['uname'].'回复了您的评论。</a>';
				}
				...
				$message['tid'] = $data['tid'];
				$message['cid'] = I('post.cid',0,'intval');
				$message['fromuname'] = $data['uname'];
	...
	public function myContentUpdate(){
		...
		if($topics->create()){
			...
			if($topics->where("id = $tid AND token = '$token' AND uid = '$wecha_id' AND status = 1")->setField($data)){
				$this->redirect("Forum/myContent",array('wecha_id'=>$data['uid'],'token'=>$data['token']));
			}
		}else{
			$this->error('系统错误');
		}
