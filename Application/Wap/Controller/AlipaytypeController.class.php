...
class AlipaytypeController extends BaseController{
	public $token;
	public $wecha_id;
	public $alipayConfig;
	...
	public function pay(){
		...
		$notify_url = C('site_url').U('Wap/Alipaytype/notify_url');
		//需http://格式的完整路径，不能加?id=123这类自定义参数
		//页面跳转同步通知页面路径
		$return_url = C('site_url').U('Wap/Alipaytype/return_url',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'],'from'=>$_GET['from']));		
	...
	public function setconfig(){
		$AlipayConfig['partner']		= trim($this->alipayConfig['pid']);
		//安全检验码，以数字和字母组成的32位字符
		$AlipayConfig['key']			= trim($this->alipayConfig['key']);
		//↑↑↑↑↑↑↑↑↑↑请在这里配置您的基本信息↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
		//签名方式 不需修改
		...
		$AlipayConfig['cacert']    = getcwd().'\\ThinkPHP\\Library\\Org\\Alipay\\cacert.pem';
	...
	public function return_url (){
		...
		//if($verify_result) {
			...
			if( I('get.trade_status') == 'TRADE_FINISHED' ||  I('get.trade_status') == 'TRADE_SUCCESS') {
				...
				$this->redirect('Wap/'.$from.'/payReturn',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'],'orderid'=>$out_trade_no));
			}else {
			  exit('付款失败');
			}
		//}else {
