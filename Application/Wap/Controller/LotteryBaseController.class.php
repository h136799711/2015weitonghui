...
class LotteryBaseController extends WapController{
	public function _initialize(){
		parent::_initialize();
		$infoWhere['wecha_id']=I('get.wecha_id');
		$infoWhere['token']=I('get.token');
	...
	public function prizeHandle($token,$wecha_id,$Lottery){
		...
		if ($Lottery['enddate'] < $now) { //过期
				...
				//最多中奖次数maxPrizeCount
				// sendTextToFan(getToken(),$wecha_id,"中奖$prizedCount次数.");
				if ($prizedCount >= $maxPrizeCount){
					$data['end'] = 3;
					$data['msg'] = "您已经中了很多奖品了，不能参加了 ^ ^ !";
					...
					//是否已经够次数
					//小于等于0时不限制可以抽奖次数
					if ($Lottery['canrqnums']  > 0 && $record['usenums'] >= $Lottery['canrqnums']  ) {
						$data['end'] = -1;
						...
						$month=date('m',$now);
						$day=date('d',$now);
						$firstSecond=mktime(0,0,$month,$day,$year);
	...
	protected function get_prize($Lottery){
		...
		if (intval($prizetype)&&$prizetype<7){
			//M('Lottery_record')->where(array('lid'=> $id,'wecha_id'=>I('get.wecha_id')))->save(array('islottery'=>1));
		}
	...
	public function add(){
		...
		$this->lottory_db=M('Lottery');
		 if($_POST['action'] ==  'add' ){
			$lid 				= I('post.lid');
			$wechaid 			= I('post.wechaid');
			//$data['sn']			= I('post.sncode');
			$data['phone'] 		= I('post.tel');
			//$data['prize']		= I('post.winprize');
			// $data['wecha_name'] = I('post.wxname');
			//$data['time']		= time(); 
			//$data['islottery']	= 1;
			//$data['usenums']	= 1;
	...
	public function exchange(){
		...
		$this->lottory_db=M('Lottery');
		 if(IS_POST){
		 	...
		 	if ($Lottery['parssword']!=trim(I('post.parssword'))){
