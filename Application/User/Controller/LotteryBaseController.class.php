...
class LotteryBaseController extends UserController{
	...
	public function sn($type){
		...
		$id=intval(I('get.id'));
		$sendstatus=intval(I('post.sendstatus'));
		$data=M('Lottery')->where(array('token'=>$this->token,'id'=>$id,'type'=>$type))->find();
		$this->assign('thisLottery',$data);

		$recordWhere['token']=$this->token;
		$recordWhere['lid'] = $id;
		$recordWhere['islottery'] = 1;
		if(!is_null(I('post.sendstatus'))  && $sendstatus != 2){
			$recordWhere['sendstatus'] = $sendstatus;
		}
		if(!is_null(I('post.sncode'))){
		$recordWhere['sn'] = array('like','%'.I('post.sncode').'%');
		}
