...
//        $this -> sendHistory();
    ...
    public function index(){
        ...
        if (IS_POST){
            $row = array();
            $row['msgtype'] = I('post.msgtype');
            $row['mediasrc'] = I('post.mediasrc');
            $row['text'] = I('post.text');
            $row['imgids'] = I('post.imgids');
    ...
    public function tosendAll(){
        // if (IS_POST){
            ...
            $id = M('Send_message') -> add($row);
            $sendrt = $this -> curlPost('https://api.weixin.qq.com/cgi-bin/message/mass/sendall?access_token=' . $postMedia['access_token'], '{"filter":{"group_id":"' . I('get.wechatgroupid') . '"},"mpnews":{"media_id":"' . $row['mediaid'] . '"},"msgtype":"mpnews"}');
            if($sendrt['rt'] == false){
    ...
    public function send(){
        $fans = M('WechatGroupList') -> where(array('token' => $this -> token)) -> order('id ASC') -> select();
        $i = intval($_GET['i']);
        $count = count($fans);
...
public function img(){
    ...
    if ($list){
        foreach ($list as $item){
            array_push($items, array('id' => $item['id'], 'name' => $item['title'], 'pic' => $item['pic'], 'info' => $item['text'], 'linkcode' => '{siteUrl}/index.php/Wap/Index/content?token=' . $this -> token . '&wecha_id={wechat_id}&id=' . $item['id'], 'linkurl' => '', 'keyword' => $item['keyword']));
        }
    }
