...
class StoreController extends UserController
{
    ...
    public function catSet() {
        $id = I('get.id');
        $checkdata = M('ProductCat')->where(array('id' => $id))->find();
        if (empty($checkdata)) {
        ...
        if (IS_POST) {
            ...
            if ($data->create()) {
                if ($data->where($where)->save($_POST)) {
                    if (!$this->isDining) {
                        $this->success('修改成功', U('Store/index', array('token' => session('token'), 'parentid' => I('post.parentid'))));
                    } else {
                        $this->success('修改成功', U('Store/index', array('token' => session('token'), 'parentid' => I('post.parentid'), 'dining' => 1)));
                    }
                } else {
                    $this->error('操作失败');
                }
    ...
    public function catDel() {
        ...
        if (IS_GET) {
            ...
            $productsOfCat = $data->where(array('parentid' => $id))->select();
            if (count($productsOfCat)) {
                $this->error('本分类下有子分类，请删除子分类后再删除分类', U('Store/index', array('token' => session('token'), 'dining' => $this->isDining)));
            }
    ...
    public function norms() {
        ...
        if ($checkdata = M('ProductCat')->where(array('id' => $catid, 'token' => session('token')))->find()) {
    ...
    public function normsAdd() {
        ...
        if ($data = M('ProductCat')->where(array('id' => I('get.catid'), 'token' => session('token')))->find()) {
    ...
    public function attributeAdd() {
        if ($checkdata = M('ProductCat')->where(array('id' => I('get.catid'), 'token' => session('token')))->find()) {
            $this->assign('catData', $checkdata);
        } else {
            $this->error("没有选择相应的分类.", U('Store/index'));
        }
        if (IS_POST) {
            ...
            $data = M('Attribute')->where(array('id' => I('get.id'), 'token' => session('token'), 'catid' => I('get.catid')))->find();
            $this->assign('catid', I('get.catid'));
            $this->assign('token', session('token'));
            $this->assign('set', $data);
            $this->display();
    ...
    public function product() {
        $catid = intval($_GET['catid']);
        $product_model = M('Product');
        $product_cat_model = M('ProductCat');
    ...
    public function addNew() {
        $catid = intval($_GET['catid']);
        $id = intval($_GET['id']);
        if ($productCatData = M('ProductCat')->where(array('id' => $catid, 'token' => session('token')))->find()) {
    ...
    public function orders() {
        ...
        if (IS_POST) {
            ...
            $unHandledCount = $product_cart_model->where(array('token' => session('token'), 'handled' => 0, 'groupon' => 0))->count();
    ...
    public function orderInfo() {
        ...
        $this->product_cat_model = M('ProductCat');
        ...
        if (IS_POST) {
            ...
            if (intval($_POST['paid']) && intval($thisOrder['price'])) {
                ...
                $set_exchange = M('MemberCardExchange')->where(array('cardid' => intval($thisCard['id'])))->find();
    ...
    public function getCat($carts) {
        ...
        if ($cartIds) {
            ...
            //商品分类
            $catdata = M('ProductCat')->where(array('id' => array('in', $cartIds)))->select();
        ...
        if ($dids) {
            ...
            foreach ($detail as $row) {
                ...
                $data[$row['pid']]['totalPrice'] = isset($data[$row['pid']]['totalPrice']) ? intval($data[$row['pid']]['totalPrice'] + $row['count'] * $row['price']) : $row['count'] * $row['price'];
                 //array('total' => $totalCount, 'totalPrice' => $totalFee);
        ...
        if (count($ids)) {
            ...
            foreach ($tmp as $row) {
                if (isset($detail_list[$row['id']])) {
                    if ($catData = M('ProductCat')->where(array('id' => $row['catid']))->find()) {
     ...
     */
    public function saveTplConfig() {
        
        if (!isset($_POST['listtpl'])) {
            return false;
        }
        $wxuser = M('wxuser');
        ...
        if (!isset($config[$this->configname])) {
            $config[$this->configname] = array();
        }
        
        // $config[$this->configname]['indextpl'] = $_POST['indextpl'];
        $config[$this->configname]['listtpl'] = $_POST['listtpl'];
        
        // var_dump($_POST['listtpl']);
        // $config[$this->configname]['indextpl'] = $_POST['indextpl'];
    ...
    public function setting() {
        ...
        if (IS_POST) {
            $saveTplResult = $this->saveTplConfig();
            ...
            if (!empty($this->wxuser['config']) && isset($this->wxuser['config'])) {
                $config = unserialize($this->wxuser['config']);
                $listtpl = $config['store']['listtpl'];
            }
            if (empty($listtpl)) {
                $listtpl = 0;
            }
