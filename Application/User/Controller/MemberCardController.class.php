...
class MemberCardController extends UserController{
	...
	public function _initialize() {
		...
		//
		$this->member_card_set_db=M('MemberCardSet');
		//
		$id=intval($_GET['id']);
		if ($id){
		...
		if ($cardByToken){
			...
			M('MemberCardExchange')->where('token=\''.$this->token.'\' AND cardid=0')->save(array('cardid'=>$cardByToken['id']));
			M('MemberCardCoupon')->where('token=\''.$this->token.'\' AND cardid=0')->save(array('cardid'=>$cardByToken['id']));
			M('MemberCardVip')->where('token=\''.$this->token.'\' AND cardid=0')->save(array('cardid'=>$cardByToken['id']));
			M('MemberCardIntegral')->where('token=\''.$this->token.'\' AND cardid=0')->save(array('cardid'=>$cardByToken['id']));
		}
		//transfer end
	...
	public function index(){
		...
		if ($cards){
			$card_create_data=M('MemberCardCreate');
			$i=0;
	...
	public function replyInfoSet(){
		...
		$memberConfig=$ReplyInfo_db->where($where_member)->find();
		$where_unmember=array('token'=>$this->token,'infotype'=>'membercard_nouse');
		$unmemberConfig=$ReplyInfo_db->where($where_unmember)->find();
		if (IS_POST){
			$memberArr=array();
			$nomemberArr=array();
			...
			//
			if ($memberConfig){
				$ReplyInfo_db->where($where_member)->save($memberArr);
			}else {
				$ReplyInfo_db->add($memberArr);
			}
			//
			if ($unmemberConfig){
				$ReplyInfo_db->where($where_unmember)->save($nomemberArr);
			}else {
				$ReplyInfo_db->add($nomemberArr);
			}
			$this->success('操作成功');
		}else {
			if (!$memberConfig){
				$memberConfig['picurl']=rtrim(C('site_url'),'/').__ROOT__.'/Public/static/images/member.jpg';
				$memberConfig['title']='会员卡,省钱，打折,促销，优先知道,有奖励哦';
				$memberConfig['info']='尊贵vip，是您消费身份的体现,会员卡,省钱，打折,促销，优先知道,有奖励哦';
			}
			if (!$unmemberConfig){
				$unmemberConfig['picurl']=rtrim(C('site_url'),'/').__ROOT__.'/Public/static/images/vip.jpg';
	...
	public function design(){
		...
		if(IS_POST){
			...
			if($data==false){				
				$this->all_insert('MemberCardSet');
			}else{
				$_POST['id']=$data['id'];
				$this->all_save('MemberCardSet');
			}
		}else{
			if($data==false){
				$data=array (
	...
	public function create_add(){ 
		$card=M("Wxuser")->where(array('uid'=>session('uid'),'token'=>session('token')))->find();
	...
	*/
	public function exchange(){
		$data=M('MemberCardExchange')->where(array('token'=>$this->token,'cardid'=>$this->thisCard['id']))->find();
		if(IS_POST){
			$_POST['token']=$this->token;
			$_POST['cardid']=$this->thisCard['id'];
			$_POST['create_time'] = time();		
			if($data==false){				
				$this->all_insert('MemberCardExchange','/exchange?token='.$this->token.'&id='.$this->thisCard['id']);
			}else{
				$_POST['id']=$data['id'];
				$this->all_save('MemberCardExchange','/exchange?token='.$this->token.'&id='.$this->thisCard['id']);
			}
		}else{
	...
	public function notice(){
		$member_card_notice_db=M('MemberCard_notice');
	...
	public function privilege(){
		$member_card_vip=M('MemberCardVip');
		
		$data=M('MemberCardVip')->where(array('token'=>$this->token,'cardid'=>$this->thisCard['id']))->order('id desc')->select();
	...
	public function privilege_del(){
		...
		if($data==false){
			...
			$this->success('操作成功',U('MemberCard/privilege',array('id'=>$this->thisCard['id'],'token'=>$this->token)));

		}
	}
	//会员优惠卷
	public function coupon(){
		$member_card_coupon_db=M('MemberCardCoupon');
	...
	public function coupon_edit(){
		...
		if(IS_POST){
			$_POST['cardid']=$this->thisCard['id'];
			if (!isset($_GET['itemid'])){
				$this->all_insert('MemberCardCoupon','/coupon?id='.$this->thisCard['id']);
			}else {
				$this->all_save('MemberCardCoupon','/coupon?id='.$this->thisCard['id']);
			}
		}else{
			$now=time();
			if (isset($_GET['itemid'])){
	...
	public function coupon_del(){
		$this->_isUseRecordExist(3,$_GET['itemid']);
			$data=M('MemberCardCoupon')->where(array('token'=>session('token'),'id'=>I('get.itemid')))->delete();
			if($data==false){
				$this->error('没删除成功');
			}else{
				$this->success('操作成功',U('MemberCard/coupon',array('id'=>$this->thisCard['id'])));
			
			}
	}
	//会员礼卷
	public function integral(){
		$member_card_inergral_db=M('MemberCardIntegral');
	...
	public function integral_edit(){
		...
		if(IS_POST){
			$_POST['cardid']=$this->thisCard['id'];
			if (isset($_GET['itemid'])){
				$this->all_save('MemberCardIntegral','/integral?id='.$this->thisCard['id']);
			}else {
				$this->all_insert('MemberCardIntegral','/integral?id='.$this->thisCard['id']);
			}
		}else{
			$now=time();
			if (isset($_GET['itemid'])){
	...
	public function integral_del(){
		$this->_isUseRecordExist(2,$_GET['itemid']);
			$data=M('MemberCardIntegral')->where(array('token'=>session('token'),'id'=>I('get.itemid')))->delete();
			if($data==false){
				$this->error('服务器繁忙请稍后再试');
			}else{
				$this->success('操作成功',U('MemberCard/integral',array('id'=>$this->thisCard['id'])));
	...
	public function useRecords(){
		...
		if ($list){
			...
			$cards=M('MemberCardCreate')->where($userinfo_where)->select();
			$cardsArr=array();
			if ($cards){
				foreach ($cards as $u){
	...
	public function members(){
		$card_create_db=M('MemberCardCreate');
		$where=array();
	...
	public function memberExpense(){
		...
		if (IS_POST){
			...
			$arr['cat']=intval(I('post.cat'));
			$arr['staffid']=intval(I('post.staffid'));
			$arr['usecount']=0;
			//
			$set_exchange = M('MemberCardExchange')->where(array('cardid'=>intval($this->thisCard['id'])))->find();
			$arr['score']=intval($set_exchange['reward'])*$arr['expense'];
			//
			M('MemberCard_use_record')->add($arr);
			$userArr=array();
	...
	public function member_del(){
		...
		//
		$where=array('wecha_id'=>$thisUser['wecha_id'],'token'=>$this->token);
		M('MemberCard_sign')->where($where)->delete();
		M('MemberCard_use_record')->where($where)->delete();
	...
	public function delete(){
		...
		if (M('MemberCardCreate')->where($tokenWhere)->find()){
			$this->error('请先删除创建的卡号');
		}
		M('MemberCardSet')->where(array('token'=>$this->token,'id'=>intval($_GET['id'])))->delete();
		$this->success('操作成功');
	}
	...
	public function getuserinfo(){
		...
		$uinfo = M('Userinfo')->where(array('wecha_id'=>$wecha_id ,'token'=>session('token')))->order('id DESC')->find();
	...
	public function info(){
		$data=M('MemberCard_info')->where(array('token'=>session('token')))->find();
		if(IS_POST){
			//dump($_POST);EXIT;
			$_POST['token']=session('token');			
			if($data==false){				
				$this->all_insert('MemberCard_info','/info');
			}else{
				$_POST['id']=$data['id'];
				$this->all_save('MemberCard_info','/info');
			}
		}else{
			$this->assign('info',$data);
			$contact=M('MemberCard_contact')->where(array('token'=>session('token')))->order('sort desc')->select();
	...
	public function contact(){
		if(IS_POST){			
			$this->all_insert('MemberCard_contact','/info');
		}else{
			$this->error('非法操作');	
		}
	}
	public function contact_edit(){
		if(IS_POST){			
			$this->all_save('MemberCard_contact','/info');
		}else{
			$this->error('非法操作');			
		}		
	}
