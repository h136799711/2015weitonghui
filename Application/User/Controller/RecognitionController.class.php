...
class RecognitionController extends UserController{
	public function _initialize(){
		parent::_initialize();
		$diymen=M('DiymenSet')->where(array('token'=>session('token')))->find();
		if($diymen ==false){
	...
	public function get_code(){
			$where=array('id'=>I('get.id','intval'),'token'=>session('token'));
			$GetDb=M('Recognition');
			$recognition=$GetDb->where($where)->field('id')->find();
			if($recognition == false) $this->error('非法操作');
			//查询appid appkey是否存在
			$api=M('DiymenSet')->where(array('token'=>session('token')))->find();
			//dump($api);
