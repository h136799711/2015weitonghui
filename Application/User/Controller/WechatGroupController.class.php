...
class WechatGroupController extends UserController{
	...
	public function _initialize() {
		...
		if ($this->thisWxUser['winxintype']!=3){
			$this->error('只有微信官方认证的高级服务号才能使用本功能',U('User/Index/edit',array('id'=>$this->thisWxUser['id'])));
		}
	}
	public function index(){
		...
		if (IS_POST&&strlen(trim($_POST['keyword']))){
			...
			$list=$group_list_db->where($where)->limit($page->firstRow.','.$page->listRows)->order('subscribe_time desc')->select();
			
			$this->assign('page',$page->show());
		}
		
		//
		$wechat_group_db=M('WechatGroup');
		//
	...
	public function  send(){
		if(IS_GET){
			...
			foreach($arrayData as $data){
				$check=M('WechatGroupList')->field('openid')->where(array('openid'=>$data))->find();
				if($check==false){
					M('WechatGroupList')->data(array('openid'=>$data,'token'=>$this->token))->add();
					$a++;
				}else{
					$b++;
				}
			...
			if (strlen($nextOpenID)){
				$this->success('本次更新'.$a.'条,重复'.$b=$b==1?0:$b.'条，正在获取下一批粉丝数据',U('User/WechatGroup/send',array('token'=>$this->token,'next_openid'=>$nextOpenID)));

			}else {
				$this->success('更新完成,现在获取粉丝详细信息',U('User/WechatGroup/send_info',array('token'=>$this->token)));
	...
	public function  send_info(){
		if(IS_GET){
			...
			$access_token=$this->_getAccessToken();
			if ($refreshAll){
				$fansCount=M('WechatGroupList')->where(array('token'=>session('token')))->count();
				$i=intval($_GET['i']);
				$step=20;
				$fans=M('WechatGroupList')->where(array('token'=>session('token')))->order('id DESC')->limit($i,$step)->select();
				if ($fans){
					...
					$this->success('更新中请勿关闭...进度：'.$i.'/'.$fansCount,U('User/WechatGroup/send_info',array('i'=>$i,'all'=>1,'token'=>$this->token)));
				}else {
					$this->success('更新完毕',U('User/WechatGroup/index',array('token'=>$this->token)));

					exit();
				}
			}else {
				$dataAll=M('WechatGroupList')->field('openid,id')->where(array('token'=>session('token'),'subscribe_time'=>''))->order('id desc')->limit(20)->select();
				if($dataAll ==false){
					$this->success('更新完毕',U('User/WechatGroup/index',array('token'=>$this->token)));
					exit();
				}
				...
				$count=M('WechatGroupList')->field('id')->where(array('token'=>session('token'),'subscribe_time'=>''))->count();
				$this->success('还有'.$count.'个粉丝信息没有更新,<br />请耐心等待',U('WechatGroup/send_info'));
			}
		}else{
			$this->error('非法操作');
		}
	...
	public function setGroup(){
		if (IS_POST){
			...
			$this->success('设置完毕',U('User/WechatGroup/index',array('token'=>$this->token)));
	...
	function sysGroups(){
		...
		//
		$this->success('操作成功',U('WechatGroup/groups'));
	}
	function groupSet(){
		$wechat_group_db=M('WechatGroup');
