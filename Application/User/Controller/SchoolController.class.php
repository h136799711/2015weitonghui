...
class SchoolController extends UserController{
    ...
    public function index(){
        ...
        if(IS_POST){
            ...
            foreach ($new_item as $k => $v) {
                $data['sname']  =   filter_var(trim($v['sname']),FILTER_SANITIZE_STRING) ? $v['sname'] :$this->error('检查是否有特殊符号或者留空项,请不要重复提交',U('School/index',array('token'=>session('token'),'type'=>I('get.type'))));
                $data['ssort']  = filter_var($v['ssort'],FILTER_VALIDATE_INT) ? $v['ssort'] : $this->error('检查是否是整数',U('School/index',array('token'=>session('token'),'type'=>I('get.type'))));
                if(empty($data['ssort'])){$data['ssort'] = 1;}
                $data['type']   = filter_var(I('get.type'),FILTER_SANITIZE_STRING) ? I('get.type') : $this->error('参数错误',U('School/index',array('token'=>session('token'),'type'=>I('get.type'))));
            ...
            unset($_REQUEST);
            if($ok){
                $this->success('添加成功',U('School/index',array('token'=>session('token'),'type'=>I('get.type'))));
                exit;
            }else{
                $this->error('添加失败',U('School/index',array('token'=>session('token'),'type'=>I('get.type'))));
                exit;
            }
        }
    ...
    public function student_score(){
        ...
        $_sid =filter_var(I('get.id'),FILTER_VALIDATE_INT);
