...
class MapController extends Controller{
	...
	public function staticCompanyMap(){
		...
		if ($companies){
			...
			foreach ($companies as $thisCompany){
				...
				$return[]=array($i.'.'.$thisCompany['name'].'地图',"电话：".$thisCompany['tel']."\r\n地址：".$thisCompany['address']."\r\n点击查看详细",$imgUrl,C('site_url').U('Wap/Company/map',array('companyid'=>$thisCompany['id'],'token'=>$this->token)));
				$i++;
			}
			//使用操作
	...
	public function walk($x,$y,$companyid=1){
		...
		if (is_array($rt)){
			...
			$return[]=array('步行到'.$thisCompany['name'].'行程有'.$distanceStr.',大概需要'.$durationStr,"具体方案：".$stepStr,$imgUrl,C('site_url').U('Wap/Company/walk',array('companyid'=>$thisCompany['id'],'latitude'=>$x,'longitude'=>$y,'token'=>$this->token)));
	...
	public function drive($x,$y,$companyindex=1){
		...
		if (is_array($rt)){
			...
			$return[]=array('开车到'.$thisCompany['name'].'行程有'.$distanceStr.',大概需要'.$durationStr,"具体方案：".$stepStr,$imgUrl,C('site_url').U('Wap/Company/drive',array('companyid'=>$thisCompany['id'],'latitude'=>$x,'longitude'=>$y,'token'=>$this->token)));
	...
	public function bus($x='',$y='',$companyindex=1){
		...
		if (is_array($rt)){
			...
			$return[]=array('坐公交到'.$thisCompany['name'].'行程有'.$distance.',大概需要'.$duration,"推荐线路：\r\n".$schemeStr,$imgUrl,C('site_url').U('Wap/Company/bus',array('companyid'=>$thisCompany['id'],'latitude'=>$x,'longitude'=>$y,'token'=>$this->token)));
	...
	public function nearest($x,$y){
		...
		if ($companies){
			...
			$return[]=array('最近的是'.$nearestCompany['name'].'，大约'.$distanceStr,"回复“步行去".$index."”“坐公交".$index."”或“开车去".$index."”获取详细路线图",$imgUrl,C('site_url').U('Wap/Company/map',array('companyid'=>$nearestCompany['id'],'token'=>$this->token)));
